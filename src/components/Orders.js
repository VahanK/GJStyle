...
export default function Orders() {
    ...
    useEffect(() => {
        fetch(`http://0000:8055/items/Order?fields=*,order_details.*,order_details.product.*,order_details.product.category.*,order_details.product.images.*,images.url&filter[user][_eq]=${userId}`)
                                        ...
                                        <ul role="list" className="divide-y divide-gray-200 px-4 sm:px-6 lg:px-8">
                                            {order.order_details.map((detail, index) => (
                                                <li key={detail.id} className="flex py-8 text-sm sm:items-center">
                                                    <img
                                                        src={`http://localhost:8055/assets/${detail.product.images[0].directus_files_id}`}
                                                        alt={`http://localhost:8055/assets/${detail.product.images[0].directus_files_id}`}
